...
function archroot {
	...
	arch-chroot /mnt bash -c "passwd && useradd --create-home $uname && echo 'set user password' && passwd $uname && groupadd sudo && gpasswd -a $uname sudo && EDITOR=nano visudo && exit"
...
function chaotic-aur {
	...
	case "$chaotic" in 
	     [Yy][eE][sS]|yY)
		    arch-chroot bash -c "pacman-key --recv-key FBA220DFC880C036 --keyserver keyserver.ubuntu.com && pacman-key --lsign-key FBA220DFC880C036 && 
...
function step-installation {
	...
	array=(set-time partion mounting base archroot set-timezone de installgrub graphics installsteam additional chaotic browser)
